...
| `shannonstore.api.admin.socket.enabled` | `true` | Whether the API node exposes the local Unix-domain admin recovery socket used by `bin/shannonstore-cli.sh` (`ping`, `iam:reset-password`). Authentication is the socket's mode `600`; never exposed on the network. See [Admin Password Recovery](admin-password-recovery.md). |
| `shannonstore.api.admin.socket.path` | (empty) | Filesystem path of the recovery socket. Empty means derive it from the IAM store — `<parent of shannonstore.api.iam.rocksdb.dir>/admin.sock` — which keeps the socket beside the IAM state this node owns. `$SHANNONSTORE_ADMIN_SOCKET` overrides both. |
| `shannonstore.api.admin.socket.marker.file` | `api.socket` | Name of the file under `<shannonstore.home>/bin` where the API node writes the socket path it actually bound to; `bin/shannonstore-cli.sh` prefers it over guessing candidate locations. Removed on shutdown. |
...
| `shannonstore.api.s3.select.max.object.bytes` | `134217728` (128 MiB) | Largest object [SelectObjectContent](s3-compatible-api.md#s3-select) will scan. A Select reads the whole object into memory to evaluate over it; a larger object is refused with `ObjectTooLarge` rather than attempted. |
| `shannonstore.api.s3.select.max.record.bytes` | `1048576` (1 MiB) | Longest single record (one CSV line, one JSON value) a Select will accept. Keeps a file with no line breaks &mdash; a truncated upload, a generated one-line file &mdash; from turning one request into an allocation the size of the object. |
| `shannonstore.api.s3.select.max.fields` | `4096` | Most fields one record may have before it is treated as malformed. Bounds CSV splitting: a line of nothing but delimiters would otherwise produce one list entry per byte. |
| `shannonstore.api.replication.ssec.scan.limit` | `1000` | How many objects `PutBucketReplication` looks at before warning that the rule covers [SSE-C](s3-compatible-api.md#sse-c) data, which replication cannot move. The warning only needs to distinguish *some* from *none*, so the scan stops here rather than walking a bucket of millions for a log line. Raise it to make that count exact on larger buckets; lower it if `PutBucketReplication` feels slow. |
| `shannonstore.api.maintenance.retry.after.seconds` | `30` | `Retry-After` sent with the `503` a request gets while the cluster is in [maintenance mode](maintenance.md). Set it to roughly how long a window usually lasts: too low and clients hammer a cluster that is deliberately closed, too high and they stay away well after it reopens. |
