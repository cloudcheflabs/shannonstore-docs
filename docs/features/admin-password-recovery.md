...
docker exec shannonstore-api /app/bin/shannonstore-cli.sh iam:reset-password
```
...
```bash
NEW_PW=$(docker exec shannonstore-api /app/bin/shannonstore-cli.sh iam:reset-password)
```
