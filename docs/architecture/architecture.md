...
External Access Layer (Proxy) is the front-end gateway that serves as the single entry point for all external S3 clients (AWS CLI, SDKs, etc.).

* Role: Handles Load Balancing, SSL Termination, and request routing.
* Key Feature: Aggregates multiple API Servers into a single endpoint. It ensures high availability by automatically bypassing API nodes that are restarting or
...
* Security (KMS & IAM): Enforces granular Role-Based Access Control (RBAC) and ensures all data is encrypted at rest and in transit using a distributed Key Management Service.
